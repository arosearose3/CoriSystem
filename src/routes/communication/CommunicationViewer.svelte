<script>
  ...
  async function loadMessages() {
    ...
    try {
      loading = true;
      const response = await fetch(`${base}/api/communication?recipient=Practitioner/${practitionerId}&_sort=-sent`);
  ...
  async function markAsRead(messageId) {
    ...
    try {
      const response = await fetch(`${base}/api/communication/${messageId}`, {
        method: 'PATCH',
        headers: {
